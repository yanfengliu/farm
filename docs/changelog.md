...
- Added Farm Notes: press `N`, click any world detail or press `Enter` to capture the canvas center, write a comment, and keep a numbered camera-aware pin with edit, view, copy, export, confirmed delete, reset-context, local persistence, exact state/camera/coordinate capture, evidence crop, and public debug-bundle support. Drafts and edits survive panel changes, invalid saved captures are dropped at a deep validation boundary, and empty comments receive an inline error. The LLM visual player can now discover labelled textareas, enter bounded freeform comments only through compatible action kinds, pin a note, reopen its panel, and reselect its world marker through visible controls.
