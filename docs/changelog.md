...
- Added Point and Box modes to Farm Notes. Box mode supports reverse and off-canvas drags, centered keyboard capture, persistent camera-aware pixel outlines, undistorted evidence previews, strict coordinate validation, legacy point-note compatibility, debug/LLM context, and responsive desktop layouts. The deterministic visual player now discovers both modes, performs a real bounded drag, requires observed draft evidence before advancing, releases the pointer after failed drags, and keeps annotation gestures separate from gameplay-paint coverage.
