...
- Fixed Farm Notes keyboard entry so `W`, `A`, `S`, and `D` type normally in draft and edit textareas while the same keys continue to pan the camera when gameplay owns keyboard focus.
