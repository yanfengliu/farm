...
- Surfaced by: recursive runs `farm-visual-loop-2026-07-13T06-14-30-782Z`, `farm-visual-loop-2026-07-13T06-24-21-415Z`, and `farm-visual-loop-2026-07-13T06-36-31-132Z` all capped with an active basket even as each repair advanced farther through the new progression.
- Failure mode: treating every stored crop as reserved deadlocked a full bin, but treating every positive surplus as continuously sellable drained extra crops after enough space was free. Generic milestone seed priority bought tomatoes while a request was one carrot short and carrot seed stock was zero. Finally, unconditional Pin actions started fourth and fifth baskets after the three-delivery milestone, consuming the budget that should have watched Tier 4 settle.
- Fix commits: `5124d86` introduced visible basket reserve accounting; `ec614d6` limited pressure sales, prioritized basket-deficit seeds, and closed Unpin lifecycle state; the curriculum follow-up intentionally abandons one unready basket and stops pinning new baskets after three completed deliveries.
- Regression anchors: `tests/browser/llmVisualLoopLocalPlayer.test.mjs` pins full-bin surplus selection, reserve preservation, the two-free-slot stop condition, request-deficit seed priority, abandoned-basket cleanup, one-time Unpin coverage, replacement retention, and the three-delivery acceptance ceiling; `tests/browser/llmVisualLoopContract.test.mjs` pins active-request-first with milestone fallback in the composed harness source.
- Behavior delta: the visual player can make space without using Sell All or consuming a request deficit, plants the crop that can actually complete the pinned basket, demonstrates the board's no-penalty choice once, and then leaves the board after its authored three-delivery progression purpose is satisfied.
