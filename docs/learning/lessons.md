...
- Current regression anchors: `tests/browser/storybookArtDirection.test.mjs` still pins presentation-only butterfly continuity and proves one 200 ms exponential worker approach equals two composed 100 ms approaches. Ducks have since become deterministic wildlife actors; the same suite now proves their state and actor pixels freeze while paused, while creek and butterfly ambience continues.
- Behavior delta: the original 13-second browser recheck reduced the worst adjacent duck-centroid change from 320.30 pixels to 10.87 pixels. The self-directed ecology follow-up removes that decorative duck route entirely, so duck translation now follows saved habitat travel and simulation time; worker interpolation remains invariant to equivalent frame partitions.
