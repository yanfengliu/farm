...
Storybook chrome accents stay layout-neutral: panel headings carry a stepped harvest-gold underline flourish, HUD chips carry a subtle stepped pixel frame, and the Coins chip pulses a brief gold glow when the total changes — all drawn with backgrounds, inset shadows, and one-shot animations inside existing padding so no text metric moves at either supported desktop size.
...
Foliage volume must come from small asymmetric leaf sprays with visible gaps, stems, species-specific structure, and varied cluster rhythm rather than broad opaque green rectangles. Cultivated crops follow the same readable organic grammar: carrots keep separate tapered blades and roots, wheat bends along narrow stalks into separated grain heads, and fruiting vines keep leaves and stepped produce distinct. Creek-edge plants use tapered blades and clustered species rhythms rather than evenly alternating stamps. Stones use asymmetric stepped row silhouettes, broken shadows, multiple mineral faces, cracks, and sparse lichen rather than grey rectangular slabs. Translucent smoke uses broken one-pixel rows, varied contours, and lateral drift rather than a column of grey tiles. These rules apply to the composited authored object, not only to each individual draw call: overlapping tiny primitives must not tile into a solid block.

The farmhands are named residents, not anonymous units: Fern, Alder, Poppy, and Rowan match the four authored outfit palettes by worker id. Names are projection-only content — they never enter the save — and the Inspect panel introduces each hand with a framed pixel portrait rendered from the exact same art the scene draws, so the portrait always matches the sprite in the field.

Work is celebrated with small presentation-only effects in the effects lane: planting kicks up a soil-toned dust puff, watering scatters falling droplets, and a harvest releases rising harvest-gold sparkles at the working farmhand. Effects spawn from lifetime-stat changes between snapshots, animate on presentation time, are capped, and never enter simulation state, saves, or replay; a paused farm spawns nothing new while in-flight pixels finish fading.
...
A deterministic border woodland rings the legal camera world, so panning to any edge meets forest rather than open ground. The woodland grows from a jittered lattice with density thinning from the world edge toward the framed view, side-weighted species pools (birch-led to the north, willow to the south, elder and willow west of the creek, apple and hazel to the east), and every tree's visual bounds stay inside the camera world, clear of the framed view with breathing room, and off the creek water. Woodland trees satisfy the same organic pixel grammar and are pickable by Farm Notes like any authored tree.
...
Pip and Mallow are autonomous residents rather than a decorative looping pair. Each duck has deterministic hunger, energy, activity, location, destination, travel progress, and meal count. Hungry ducks reserve distinct available fish, swim to the corresponding creek habitat, eat, and release the fish into a renewable respawn cycle. Tired ducks abandon foraging, walk to a compatible grove shelter, tuck beneath the tree canopy, sleep until rested, and then return to the creek without player direction. Creek roaming reverses at the north and south habitat ends, all travel advances at an unhurried half-rate with the pair's movement beats staggered, and longer habitat trips use smaller steps so a duck never snaps across a large part of the scene in one simulation tick.
...
Press `N` or click Note to enter a separate annotation mode without changing the selected farm tool. Clicking Point or Box in the notebook immediately arms that shape, so the next canvas interaction cannot fall through to a farm tool. Point mode captures one visible duck, worker, plant, hedgerow, building, creek detail, or tile; keyboard players can press `Enter` to capture the canvas center. Box mode is mouse-drag only: press anywhere on the canvas, drag to any freeform size above the 12 × 12 px accidental-drag threshold and any position, and release to capture the exact continuous-pixel rectangle rather than a grid cell. Its warm pixel outline and live dimensions remain visible while the comment is drafted; a tiny accidental drag is rejected, reverse-direction drags normalize to the same region, and releasing beyond the canvas still completes the gesture. Capturing pauses the farm, `Ctrl`/`Cmd` + `Enter` pins the note, and `Escape` cancels the active drag, draft, edit, or aiming state. Draft and edit text survives panel changes. Each saved note receives a stable numbered world pin or boxed region and supports View, Edit, Copy, Export, and two-step Delete; View restores the captured camera composition and highlights the selected annotation.
