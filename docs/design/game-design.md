...
Press `N` or click Note to enter a separate annotation mode without changing the selected farm tool. Point mode captures one visible duck, worker, plant, building, creek detail, or tile. Box mode drags a world-anchored rectangular region and keeps its warm pixel outline visible while the comment is drafted; a tiny accidental drag is rejected, reverse-direction drags normalize to the same region, and releasing beyond the canvas still completes the gesture. Keyboard players can press `Enter` while aiming to capture either the center point or a centered box. Capturing pauses the farm, `Ctrl`/`Cmd` + `Enter` pins the note, and `Escape` cancels the active drag, draft, edit, or aiming state. Draft and edit text survives panel changes. Each saved note receives a stable numbered world pin or boxed region and supports View, Edit, Copy, Export, and two-step Delete; View restores the captured camera composition and highlights the selected annotation.
