...
Farm Notes browser coverage must prove that a note click does not submit a farm command, keyboard aiming can capture the canvas center, draft pause cannot be bypassed by the ordinary pause control, empty-comment validation is visible, draft and edit text survives panel changes, persisted user text stays inert, malformed nested captures are dropped without a crash, exact coordinates survive camera changes, reset clears an active editor and moves saved records to past-farm context, aiming clicks through existing pins, View restores the captured camera, delete requires confirmation, and the populated editor/list plus thirteen-button toolbar fit at 1280x800 and 1024x720. The focused contract is `tests/browser/annotations.test.mjs`; pure persistence, edge-crop crosshair geometry, and target-selection behavior live in `tests/persistence/localAnnotations.test.ts`, `tests/annotations/farmAnnotationCapture.test.ts`, and `tests/phaser/farmAnnotationTarget.test.ts`.
