...
Use the Markdown report as the review packet for an LLM-player loop: inspect the screenshots, visible text, available actions, player action log, structured findings, and annotations, choose the highest-impact player-facing improvement, implement it, then rerun the harness. Available actions should include the same player-facing surface a person can use in that screenshot: canvas clicks, buttons, range sliders, numeric inputs, role buttons, resize handles, and scrollable panel regions. Action entries should carry hints and visible control state such as active toolbar buttons, keyboard shortcuts, input values, and scroll position.
