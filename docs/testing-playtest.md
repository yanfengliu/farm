...
- `window.__farmDebug.getAnnotations()` and `window.__farmDebug.getAnnotationContext()`
...
Farm Notes browser coverage must prove that a note click does not submit a farm command, keyboard aiming can capture the Point-mode canvas center, clicking either shape while idle immediately arms annotation input, and Box mode accepts only a real mouse drag with arbitrary non-grid-aligned endpoints. Reverse-direction box gestures must persist and project at the same world location after pan/zoom/reload, off-canvas release must complete a captured drag, live/draft/list UI must report area dimensions rather than presenting the center cell as the selection, and Box-mode `Enter` must leave aiming active with drag guidance instead of synthesizing a fixed region. Tiny drags and `Escape` cancel safely, draft pause cannot be bypassed by the ordinary pause control, empty-comment validation is visible, draft and edit text survives panel changes, persisted user text stays inert, malformed or cross-space-incoherent captures are dropped without a crash, exact coordinates survive camera changes, reset clears an active editor and moves saved records to past-farm context, aiming clicks through existing pins, View restores the captured camera, delete requires confirmation, evidence previews remain unstretched at high device-pixel ratio, and the populated editor/list plus toolbar fit at 1280x800 and 1024x720. Real-keyboard coverage must audit both `keydown` and `keyup` for every Phaser-registered movement key and every gameplay shortcut while the draft and edit textareas own focus; only `Escape` cancellation and `Ctrl`/`Cmd` + `Enter` save may intentionally prevent the browser default. The focused contracts are `tests/browser/annotations.test.mjs`, `tests/browser/annotationBoundingBox.test.mjs`, and `tests/browser/annotationKeyboard.test.mjs`; pure persistence, selection geometry, edge-crop preview geometry, and target-selection behavior live in `tests/persistence/localAnnotations.test.ts`, `tests/annotations/farmAnnotationSelection.test.ts`, `tests/annotations/farmAnnotationCapture.test.ts`, and `tests/phaser/farmAnnotationTarget.test.ts`.
...
Use the Markdown and HTML reports as the review packet for an LLM-player loop: inspect the screenshots, local `screenshotFile` paths, visible text, available actions, keyboard actions, player action log, structured findings, and annotations, choose the highest-impact player-facing improvement, implement it, then rerun the harness. Available actions should include the same player-facing surface a person can use in that screenshot: canvas clicks, buttons, range sliders, numeric inputs, textareas, role buttons, resize handles, scrollable panel regions, keyboard-only camera controls, selector-focused keyboard controls for focusable inputs and resize handles, and visible toolbar shortcuts such as tool, undo/redo, pause, and speed keys. Action entries should carry hints and visible control state such as active toolbar buttons, keyboard shortcuts, focus requirements, numeric values, bounded textarea values/max lengths, and scroll position. Do not cap the extracted visible actions; if a control is visible and hit-test reachable in the browser viewport, the LLM-player should see it. Do not cap the extracted player-visible text before sending it to the decision provider; normalize whitespace only, and let downstream providers handle any prompt-budget summaries. Scenario capture should wait for a rendered browser frame, sample the DOM observation, then write the PNG so the review packet describes the same moment the screenshot shows. When a frame has to be recaptured, retry that whole sequence rather than re-shooting alone: settling advances the world, so a fresh screenshot beside a stale DOM sample would break exactly that guarantee. Capture also measures opaque-black coverage and the longest unbroken black row and column before accepting a frame, because the farm authors no pure black and any opaque black region means the browser compositor, not the game, produced the image. An accepted frame carries no capture metadata; only a frame that needed a retry records `screenshotCapture` with its attempt count and metrics. If no attempt comes back clean, capture keeps the least-corrupt frame, marks it `degraded`, and the run raises a `screenshot-compositor-degraded` finding — the run's report must never be thrown away over one bad frame, and a corrupt frame must never pass review as ordinary proof.
...
Visual loop action kinds in the Farm provider schema are `click`, `hover`, `drag`, `adjust`, `type`, `wheel`, `press`, `wait`, `viewport`, and `stop`; the adapter maps them onto `civ-engine` visual actions such as `click`, `hover`, `drag`, `type`, `key`, `wheel`, `wait`, `viewport`, and `stop` for loop execution and trace redaction. Click and canvas drag actions may include `x`/`y` coordinates relative to the target element, which lets an LLM choose canvas locations from the screenshot. Hover actions move the pointer to a visible click control without activating it. Drag actions move the mouse from a canvas or resize handle by a bounded pixel delta, wheel actions move the mouse over a visible canvas or scrollable panel target before scrolling, adjust actions target only visible range/number inputs, and type actions target only an observed, enabled, hit-test-reachable textarea with nonblank text bounded by its visible `maxLength` and the provider's 2,000-character ceiling. Provider normalization rejects action kinds incompatible with the observed control hint, coverage counts a control as exercised only after successful execution, and failed executions never advance the deterministic player's curriculum state. Press actions may use only listed keyboard controls such as Arrow/WASD camera panning, Home farm recentering, Space pause, visible tool shortcuts, undo/redo, speed keys, range-input arrows, number-input up/down arrows, and side-panel resizer arrows/Home/End. Focus-required keyboard actions include a selector in the observation and must include that selector in the decision so the harness focuses the visible control before pressing. Tune local runs with `FARM_VISUAL_LOOP_STEPS`, `FARM_VISUAL_LOOP_WAIT_MS`, and `FARM_VISUAL_LOOP_SETTLE_MS`; ordinary visual loops default to 80 decisions, while `npm run playtest:recursive` supplies the 160-step Farm Notes plus Harvest Hearth curriculum unless `FARM_VISUAL_LOOP_STEPS` is explicitly set. The visual-loop boundary clamps larger budgets to the same 160-decision safety ceiling. If a capped run ends while the final visible text still contains actionable guidance, the harness should report `visual-loop-ended-with-guidance` rather than treating the run as clean.
...
Each `playtest:recursive` wrapper also freezes a self-contained proof packet under `output/playwright/llm-visual-loop-history/pass-artifacts/<pass-id>/`: stable JSON, Markdown, HTML, replay bundle, `steps/` screenshots, and `pass-manifest.json`. The archived run rewrites `bundlePath` and every `screenshotFile` to that immutable packet while preserving replay-relative `steps/...` references, so replacing the canonical latest run cannot silently change old visual evidence. A missing referenced screenshot or bundle fails and removes the partial packet instead of appending an incomplete proof.
